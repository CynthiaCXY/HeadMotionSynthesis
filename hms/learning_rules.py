...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, decay_coeff=0.9):
        super(RMSPropLearningRule, self).__init__(learning_rate)
        assert decay_coeff >= 0. and decay_coeff <= 1., (
            'decay_coeff should be in the range [0, 1].'
        )
    ...
    def initialise(self, params):
        ...
        self.sgras = []
        for param in self.params:
            self.sgras.append(np.zeros_like(param))

    def reset(self):
        for sgra in zip(self.sgras):
            sgra *= 0.          

    def update_params(self, grads_wrt_params):
        for param, sgra, grad in zip(self.params, self.sgras, grads_wrt_params):
            sgra *= self.decay_coeff
            sgra += (1 - self.decay_coeff) * grad**2
            param -= self.learning_rate * grad / (np.sqrt(sgra) + 1e-8)
            

class AdamLearningRule(GradientDescentLearningRule):

    def __init__(self, learning_rate=1e-3, decay_coeff=0.999, mom_coeff = 0.9):
    ...
    def update_params(self, grads_wrt_params):
        for param, sgra, mom, grad in zip(self.params, self.sgras, self.moms, grads_wrt_params):
            sgra *= self.decay_coeff
            sgra += (1 - self.decay_coeff) * grad**2
            mom *= self.mom_coeff
            mom += (1- self.mom_coeff) * grad
            param -= self.learning_rate * mom / (np.sqrt(sgra) + 1e-8)
