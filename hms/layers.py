...
import numpy as np
import hms.initialisers as init
...
class BatchNormalizationLayer(StochasticLayerWithParameters):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        return self.gamma*(inputs - mean)/np.sqrt(variance + self.epsilon) + self.beta
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        mean = np.mean(inputs, axis=0)
        variance = np.var(inputs, axis=0)
        with_res_to_norm = grads_wrt_outputs * self.gamma
        
        with_res_to_var = -0.5 * (variance + self.epsilon) ** (-3. /2.) * np.sum(with_res_to_norm * (inputs - mean), axis=0)
        
        with_res_to_mean = np.sum(with_res_to_norm / - np.sqrt(variance + self.epsilon), axis=0) + with_res_to_var*np.mean((inputs - mean), axis=0)* -2.
        return with_res_to_norm / np.sqrt(variance + self.epsilon) + with_res_to_var * 2. * (inputs - mean) / inputs.shape[0] + with_res_to_mean / inputs.shape[0]
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        u_hat = (inputs - mean)/np.sqrt(variance + self.epsilon)
        
        grads_wrt_weights = np.sum(grads_wrt_outputs*u_hat, axis=0)
        grads_wrt_biases = np.sum(grads_wrt_outputs, axis=0)
        return [grads_wrt_weights, grads_wrt_biases]
...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        final_output =  np.zeros((len(inputs), self.num_output_channels, output_dim_1, output_dim_2))
    
        for index, input in enumerate(inputs):
            for output_channel in range(self.num_output_channels):
                for input_channel in range(self.num_input_channels):
                    kernel = self.kernels[output_channel, input_channel]
                    final_output[index, output_channel] += correlate2d(input[input_channel], kernel, 'valid')     
                final_output[index, output_channel] +=  self.biases[output_channel]
        return final_output
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        padded_grads_wrt_outputs = np.zeros((outputs.shape[0],outputs.shape[1],outputs.shape[2] + (self.kernel_dim_1-1)*2, outputs.shape[3] + (self.kernel_dim_2-1)*2))
        
        padded_grads_wrt_outputs[:, :, self.kernel_dim_1-1:self.kernel_dim_1+grads_wrt_outputs.shape[2]-1, self.kernel_dim_2-1:self.kernel_dim_2+grads_wrt_outputs.shape[3]-1] = grads_wrt_outputs

        kernels_rotate = np.rot90(self.kernels[:, :, ::-1, ::-1])[::-1]
        
        final_output =  np.zeros((len(inputs), self.num_input_channels, self.input_dim_1, self.input_dim_2))
        for index, input in enumerate(padded_grads_wrt_outputs):
            for output_channel in range(self.num_input_channels):
                for input_channel in range(padded_grads_wrt_outputs.shape[1]):
                    kernel = kernels_rotate[output_channel,input_channel]
                    final_output[index,output_channel] += correlate2d(input[input_channel], kernel, 'valid')
        return final_output    
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        grads_wrt_biases = np.zeros(len(inputs))

        for index, input in enumerate(inputs):
            #grads_wrt_kernels_temp = np.zeros(grads_wrt_kernels.shape)
            for output_channel in range(self.num_output_channels):
                for input_channel in range(self.num_input_channels):
                    grads_wrt_kernels[output_channel, input_channel] += correlate2d(input[input_channel], grads_wrt_outputs[index, output_channel], 'valid')
                #grads_wrt_biases[output_channel] += np.sum(grads_wrt_outputs[index, output_channel])
        grads_wrt_biases = grads_wrt_outputs.sum(axis=3).sum(axis=2).sum(axis=0)
...
class MaxPoolingLayer(Layer):
    ...
    def __init__(self, num_input_channels,
        ...
        self.kernel_dim_1 = kernel_dim_1
        self.kernel_dim_2 = kernel_dim_2
        self.output_dim_1 = int(input_dim_1/kernel_dim_1)
        self.output_dim_2 = int(input_dim_2/kernel_dim_2)
    ...
    def fprop(self, inputs):
        ...
        final_output =  np.zeros((len(inputs), self.num_input_channels, output_dim_1, output_dim_2))
        for index, input in enumerate(inputs):
            for input_channel in range(self.num_input_channels):
                output = np.array(np.split(input[input_channel], output_dim_1, axis = 1))
                output = np.array(np.split(output, output_dim_2, axis = 1))
                output = np.amax(np.amax(output, axis = 2), axis = 2)
                final_output[index, input_channel] = output
                expanded_max = np.repeat(np.repeat(output, 2, axis=1), 2, axis = 0)
                self.weights[index,input_channel] = expanded_max == input[input_channel]
                
                #for r in range(0, output_dim_1):
                    #for c in range(0, output_dim_2):
                        #max_value = np.max(input[input_channel][r*self.kernel_dim_1:r*self.kernel_dim_1+self.kernel_dim_1, c*self.kernel_dim_2:c*self.kernel_dim_2+self.kernel_dim_2])
                        #final_output[index, input_channel, r,c] = max_value
                        #max_index = np.argmax(input[input_channel][r*self.kernel_dim_1:r*self.kernel_dim_1+self.kernel_dim_1, c*self.kernel_dim_2:c*self.kernel_dim_2+self.kernel_dim_2])

                        #self.weights[index][input_channel][int(max_index/self.kernel_dim_1) + r*self.kernel_dim_1][int(max_index % self.kernel_dim_2) + c*self.kernel_dim_2] = int(1)
        return final_output

    def bprop(self, inputs, outputs, grads_wrt_outputs):
        output = np.zeros(inputs.shape)
        for index, grad in enumerate(grads_wrt_outputs):
            for input_channel in range(self.num_input_channels):
                temp = np.repeat(grad[input_channel], 2, axis=1)
                temp = np.repeat(temp, 2, axis = 0)
                output[index,input_channel] = temp
